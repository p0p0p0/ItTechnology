 ...
 * 全部订单的fragment
 */
public class ToubiaoMainPagerAdater extends FragmentPagerAdapter {
    //标题数组
    private String[] title0;
    private int mCode;
    int i;

    public ToubiaoMainPagerAdater(FragmentManager fm, int i) {
        super(fm);
        this.i = i;
        mCode = (int) SharedPrefsUtil.get(ECApplication.context, "toubiao", 0);
        switch (mCode) {
            case 0:
    ...
    public Fragment getItem(int position) {
        if (i == 0) {
            return ToubiaoFragmentFactory.create(position);
        }
        return null;
    }
